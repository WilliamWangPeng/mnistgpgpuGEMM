...
do 
	...
	do
		grep -v "^GPGPU-Sim\|^[ \t]\+\|L2_BW_total\|^ID\|^$" xx10_bs${bs}_l${l} |awk -f plt.awk  > awkres_bs${bs}_l${l}
		#grep -v "^GPGPU-Sim\|^[ \t]\+\|L2_BW_total\|^ID\|^$" xx10_bs${bs}_l${l} |awk -f plt.awk|awk -f rep.awk  > awkres_bs${bs}_l${l}
	done
done 
